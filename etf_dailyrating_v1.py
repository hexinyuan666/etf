...
def main():
    ...
    try:
        ...
        if results:
            ...
            filename = f'etf_complete_rating_{datetime.now().strftime("%Y%m%d_%H%M")}.csv'
            df_results.to_csv(filename, index=False, encoding='utf-8-sig')
            print(f"💾 完整排名已保存至: {filename}")
            
            # 同时保存前100名到另一个文件
            df_top100 = df_results.head(100)
            top100_filename = f'etf_top100_rating_{datetime.now().strftime("%Y%m%d_%H%M")}.csv'
